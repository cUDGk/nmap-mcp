...
| `arp` | `-sn -PR` ショートカット (LAN 内 ARP ホスト発見、管理者権限が必要な場合あり) |
| `vuln` | `-sV --script vuln` ショートカット (NSE vuln カテゴリで CVE チェック、`script` を併せると結合される) |
| `from_xml` | 既存の nmap XML ファイル (`xml_path`) を読み込んで同じパーサにかける (オフライン解析用) |

**追加オプション** (全スキャン系で有効):
- `traceroute: true` → `--traceroute` を付与
- `save_xml: <path>` → 生 XML をファイル保存 (`from_xml` で後から解析可能)
...
```json
{"action": "arp", "target": "192.168.1.0/24", "timing": 4}
```

CVE チェック (自ホストのみが前提):

```json
{"action": "vuln", "target": "192.168.1.10",
 "ports": "22,80,443", "timing": 3}
```

結果を XML 保存 → 後でオフライン再解析:

```json
{"action": "scan", "target": "192.168.1.0/24",
 "tcp_connect": true, "top_ports": 100,
 "save_xml": "C:/scans/lan-2026-04.xml"}

{"action": "from_xml", "xml_path": "C:/scans/lan-2026-04.xml"}
```
